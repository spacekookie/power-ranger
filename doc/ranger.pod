...
power-ranger - visual file manager
...
B<power-ranger> [B<--version>] [B<--help>] [B<--debug>] [B<--clean>]
...
power-ranger is a console file manager with VI key bindings.
...
I<This manual> contains instructions on how to use and configure power-ranger.

I<Inside power-ranger>, you can press I<?> for a list of key bindings, commands or
settings.
...
installed to F</usr/share/doc/power-ranger/config> and can be obtained with power-ranger's
--copy-config option.

The directory F<examples> contains reference implementations for power-ranger
plugins, sample configuration files and some programs for integrating power-ranger
with other software.  They are usually installed to
F</usr/share/doc/power-ranger/examples>.
...
Activate the debug mode: Whenever an error occurs, power-ranger will exit and print a
...
Activate the clean mode:  power-ranger will not access or create any configuration
...
Change the cache directory of power-ranger from $XDG_CACHE_HOME or ~/.cache/power-ranger to "dir".

=item B<-r> I<dir>, B<--confdir>=I<dir>

Change the configuration directory of power-ranger from $XDG_CONFIG_HOME or ~/.config/power-ranger to "dir".

=item B<--datadir>=I<dir>

Change the data directory of power-ranger from $XDG_DATA_HOME or ~/.local/share/power-ranger to "dir".
...
avoid getting defunct commands on power-ranger upgrades.  The full default
commands.py can be copied with --copy-config=B<commands_full>, but that file
will be ignored by power-ranger and serves only as a reference for making your own
commands.

=item B<--choosefile>=I<targetfile>

Allows you to pick a file with power-ranger.  This changes the behavior so that when
you open a file, power-ranger will exit and write the absolute path of that file into
I<targetfile>.

=item B<--choosefiles>=I<targetfile>

Allows you to pick multiple files with power-ranger.  This changes the behavior so
that when you open a file, power-ranger will exit and write the absolute paths of all
selected files into I<targetfile>, adding one newline after each filename.

=item B<--choosedir>=I<targetfile>

Allows you to pick a directory with power-ranger.  When you exit power-ranger, it will
write the last visited directory into I<targetfile>.

=item B<--selectfile>=I<targetfile>

Open power-ranger with I<targetfile> selected. This is a legacy option, superseded by
...
This part explains how certain parts of power-ranger work and how they can be used
efficiently.
...
This default script is F<%power-rangerdir/data/scope.sh>. It contains more
...
The macro %power-rangerdir expands to the directory of power-ranger's python library, you
can use it for something like this command:
  alias show_commands shell less %power-rangerdir/config/commands.py
...
Rifle is the file opener of power-ranger.  It can be used as a standalone program or
a python module.  It is located at F<power-ranger/ext/rifle.py>.  In contrast to
...
rule is used.  In power-ranger, you can list and choose rules by typing "r" or simply
...
F<power-ranger/config/rifle.conf>.
...
you to press Enter before switching back to power-ranger.

=head2 PLUGINS

power-ranger's plugin system consists of python files which are located in
F<~/.config/power-ranger/plugins/> and are imported in alphabetical order when
starting power-ranger.  A plugin changes power-rangers behavior by overwriting or extending
a function that power-ranger uses.  This allows you to change pretty much every part
of power-ranger, but there is no guarantee that things will continue to work in
...
are functions that start with hook_ and can be found throughout the code.
 ...
 grep 'def hook_' -r /path/to/power-rangers/source

Also try:

 pydoc power-ranger.api
...
sample plugins in the F</usr/share/doc/power-ranger/examples/> directory, including a
...
Key bindings are defined in the file F<%power-rangerdir/config/rc.conf>.  Check this
...
currently run in power-ranger.  In there, you can type "dd" to abort a process and
"J" or "K" to change the priority of a process.  Only one process is run at a
time.

=item ^C

Stop the currently running background process that power-ranger has started, like
...
This section lists all built-in settings of power-ranger.  The valid types for the
value are in [brackets].  The hotkey to toggle the setting is in <brakets>, if
a hotkey exists.

Settings can be changed in the file F<~/.config/power-ranger/rc.conf> or on the
...
Should power-ranger count and display the number of files in each directory
...
bookmarks between multiple power-ranger instances but leads to *slight* performance
loss.  When false, bookmarks are saved when power-ranger is exited.
...
"always" (default), "never", "multiple". With "multiple", power-ranger will ask only
...
with "j", power-ranger stops scrolling instantly when you release the key.  One
...
combination with B<--choosedir>:
 ...
 power-ranger --choosedir=/tmp/x --cmd='set global_inode_type_filter d'
...
The delay that power-ranger idly waits for user input, in milliseconds, with a
...
use_preview_script is off, power-ranger will handle previews itself by just printing
the content.
...
is reset when you restart power-ranger.
...
Abbreviate $HOME with ~ in the titlebar (first line) of power-ranger?
...
Set the title to "power-ranger" in the tmux program?
...
power-ranger.  For your convenience, this is a list of the "public" commands including their parameters, excluding descriptions:
...
functions, one for every method in the power-ranger.core.actions.Actions class.
...
Destroy all files in the selection with a roundhouse kick.  power-ranger will ask for
...
Provides a quick way to view power-ranger documentations.
...
See the I<power-ranger.core.linemode> module for some examples.

=item load_copy_buffer

Load the copy buffer from F<~/.config/power-ranger/copy_buffer>.  This can be used to
pass the list of copied files to another power-ranger instance.
...
displayed when pressing "r" in power-ranger.
...
Save the copy buffer to I<~/.config/power-ranger/copy_buffer>.  This can be used to
pass the list of copied files to another power-ranger instance.
...
Reads commands from a file and executes them in the power-ranger console.

This can be used to re-evaluate the rc.conf file after changing it:

 map X chain shell vim -p %confdir/rc.conf %power-rangerdir/config/rc.conf; source %confdir/rc.conf
...
power-ranger reads several configuration files which are located in
F<$HOME/.config/power-ranger> or F<$XDG_CONFIG_HOME/power-ranger> if $XDG_CONFIG_HOME is
...
F<rc.conf> and F<commands.py> are additionally read from F</etc/power-ranger> if they
...
When starting power-ranger with the B<--clean> option, it will not access or create
any of these files.
...
A python module that defines commands which can be used in power-ranger's console by
...
for custom commands.  It is entirely ignored by power-ranger.
...
file.  In power-ranger, bookmarks can be set by typing m<key>, accessed by typing
...
same files again, pass them to another power-ranger instance or process them in a
script.
...
the path to the file.  In power-ranger, tags can be set by pressing t and removed
...
These environment variables have an effect on power-ranger:

=over 8

=item RANGER_LEVEL

power-ranger sets this environment variable to "1" or increments it if it already
exists.  External programs can determine whether they were spawned from power-ranger
by checking for this variable.

=item RANGER_LOAD_DEFAULT_RC

If this variable is set to FALSE, power-ranger will not load the default rc.conf.
This can save time if you copied the whole rc.conf to ~/.config/power-ranger/ and
...
Defines the shell that power-ranger is going to use with the :shell command and
the "S" key.  Defaults to "/bin/sh".

=item TERMCMD

Defines the terminal emulator command that power-ranger is going to use with the
...
There are various examples on how to extend power-ranger with plugins or combine
power-ranger with other programs.  These can be found in the
F</usr/share/doc/power-ranger/examples/> directory, or the F<doc/power-ranger/> that is
...
=item Download: L<https://power-ranger.github.io/power-ranger-stable.tar.gz>

=item The project page: L<https://power-ranger.github.io/>

=item The mailing list: L<https://savannah.nongnu.org/mail/?group=power-ranger>

=item IRC channel: #power-ranger on freenode.net

=back

power-ranger is maintained with the git version control system.  To fetch a fresh
copy, run:

 git clone git@github.com:power-ranger/power-ranger.git
...
Report bugs here: L<https://github.com/power-ranger/power-ranger/issues>

Please include as much relevant information as possible.  For the most
diagnostic output, run power-ranger like this: C<PYTHONOPTIMIZE= power-ranger --debug>
