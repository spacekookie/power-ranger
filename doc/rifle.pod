...
rifle - power-ranger's file opener
...
written to meet the needs of the file manager I<power-ranger>.  rifle's strength lies
...
rifle shares configuration files with power-ranger, though power-ranger is not required in
order to use rifle. The default configuration file F<rifle.conf> is expected
to be at F<~/.config/power-ranger/rifle.conf>. However, this can be overridden with
the B<-c> option.
...
with power-ranger.  To obtain it, you need to run: C<power-ranger --copy-config=rifle>
